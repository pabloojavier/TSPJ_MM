...
def padberg_rao(G:nx.Graph,
    ...
    H_prime = nx.Graph()
    for e in G.edges:
        if valoresX[e]+valoresX[(e[1],e[0])] > 0 and e[0]<e[1]:
            # print(e,valoresX[e])
            # print((e[1],e[0]),valoresX[(e[1],e[0])])
            H_prime.add_edge(e[0],e[1],capacity = c[e], weight = valoresX[e]+valoresX[(e[1],e[0])])

            # H_prime.add_edge(e[0],e[1],capacity = c[e], weight = valoresX[e])

        H.add_edge(e[0],e[1],capacity = c[e], weight = valoresX[e])
    
    # nx.draw(H_prime,with_labels=True)
    # plt.show()
    # exit(0)
...
print('Cortes Gomory Hu:',m._CORTES_GOMHU)
